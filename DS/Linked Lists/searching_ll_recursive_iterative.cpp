...
//Recursive approach
bool searchNodeRecursive(node* head,int key) {
  if(head == NULL) {
    return false;
  } else if(head->data == key) {
    return true;
  } else {
    return searchNodeRecursive(head->next, key);
  }
}
//Iterative approach for searching.

bool searchNodeIterative(node* head, int key) {
  while(head != NULL) {
    if(head->data == key) {
      return true;
    } 
...
int main() {
  ...
  if(searchNodeRecursive(head, 7)) {
    cout << "Value is present";
  } else {
    cout << "OOPS!! nothing in the list";
  }
